...
def edge_structure_entropy(data, pc, p1, p2):
    ...
    if len_left_not_zero < 2 and len_right_not_zero > 1:
        ...
        right_entropy = -np.sum(
            np.average(data[right_not_zero, :], axis=1) * right_prob * np.log2(right_prob)) / np.log2(
            len_right_not_zero)
    ...
    if len_left_not_zero > 1 and len_right_not_zero < 2:
        ...
        left_entropy = -np.sum(np.average(data[left_not_zero, :], axis=1) * left_prob * np.log2(left_prob)) / np.log2(
            len_left_not_zero)
    ...
    left_entropy = -np.sum(np.average(data[left_not_zero, :], axis=1) * left_prob * np.log2(left_prob)) / np.log2(
        len_left_not_zero)
    right_entropy = -np.sum(np.average(data[right_not_zero, :], axis=1) * right_prob * np.log2(right_prob)) / np.log2(
        len_right_not_zero)
...
for i in range(num):
    for j in range(i - 1):
        if string_bool[i, j] != 0:
            ...
            edge_origin_entropy.append(edge_structure_entropy(origin_frame, origin_pc, i, j))
    ...
    for i in range(num):
        for j in range(i - 1):
            if string_bool[i, j] != 0:
                ...
                edge_append_entropy.append(edge_structure_entropy(append_frame, append_pc, i, j))
...
landscape.to_csv("edge_structure_entropy_spearman_logM-1_" + str(string_limit) + ".csv", index=False)
