    ...
    for i in range(num):
        for j in range(i - 1):
            if string_bool[i, j] != 0:
                ...
                edge_append_entropy.append(edge_flow_entropy(new, new_pc, i, j))
...
landscape.to_csv("edge_singlesample_flow_entropy_spearman_logM-1_" + str(string_limit) + ".csv", index=False)
